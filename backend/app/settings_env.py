...
class EnvSettings:
    ...
    lab_think_tank_log_info: bool = _get("LAB_THINK_TANK_LOG_INFO", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    kalshi_use_keyring: bool = _get("KALSHI_USE_KEYRING", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    kalshi_keyring_service: str = _get("KALSHI_KEYRING_SERVICE", "kalshibot")
    kalshi_keyring_username: str = _get(
        "KALSHI_KEYRING_USERNAME", "KALSHI_PRIVATE_KEY_PEM"
    )
    ...
    data_logging_enabled: bool = _get("DATA_LOGGING", "1").lower() not in (
        "0",
        "false",
        "no",
        "",
    ...
    alert_webhook_min_seconds: float = _get_float(
        "ALERT_WEBHOOK_MIN_SECONDS", 120.0, min_value=1.0
    )
    # Longer in-process /markets and orderbook TTL right after process start to avoid a 5× cold-start storm.
    kalshi_cold_start_cache_ttl_s: float = _get_float(
        "KALSHI_COLD_START_CACHE_TTL_S", 90.0, min_value=5.0
    )
    # Steady-state defaults after warm-up (restored a few seconds after pre-warm; see kalshi_client).
    kalshi_open_markets_ttl_s: float = _get_float(
        "KALSHI_OPEN_MARKETS_TTL_S", 10.0, min_value=1.0
    )
    kalshi_orderbook_ttl_s: float = _get_float(
        "KALSHI_ORDERBOOK_TTL_S", 10.0, min_value=1.0
    )
    # Log perf_counter() phases during API lifespan (also force-enabled when KALSHI_PROFILE_STARTUP=1).
    profile_startup: bool = _get("KALSHI_PROFILE_STARTUP", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    # PHASE 2: optional Kalshi WebSocket (orderbook + ticker); REST cache remains fallback.
    kalshi_ws_enabled: bool = _get("KALSHI_WS_ENABLED", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    kalshi_ws_max_markets: int = _get_int(
        "KALSHI_WS_MAX_MARKETS", 120, min_value=1, max_value=500
    )
    kalshi_ws_subscribe_chunk: int = _get_int(
        "KALSHI_WS_SUBSCRIBE_CHUNK", 40, min_value=5, max_value=80
    )
    kalshi_ws_reconnect_cap_s: float = _get_float(
        "KALSHI_WS_RECONNECT_CAP_S", 60.0, min_value=5.0
    )
    # Log dual_engine_loop iteration wall time when interval (seconds) > 0 (e.g. 30 = log every 30s).
    kalshi_log_tick_interval_s: float = _get_float(
        "KALSHI_LOG_TICK_INTERVAL_S", 0.0, min_value=0.0
    )
    # PHASE 3: centralize remaining hot-path constants (no behavior change; defaults match prior code).
    kalshi_http_timeout_s: float = _get_float(
        "KALSHI_HTTP_TIMEOUT_S", 30.0, min_value=5.0
    )
    kalshi_http_connect_timeout_s: float = _get_float(
        "KALSHI_HTTP_CONNECT_TIMEOUT_S", 15.0, min_value=1.0
    )
    kalshi_http_max_connections: int = _get_int(
        "KALSHI_HTTP_MAX_CONNECTIONS", 40, min_value=5
    )
    kalshi_http_max_keepalive_connections: int = _get_int(
        "KALSHI_HTTP_MAX_KEEPALIVE_CONNECTIONS", 20, min_value=2
    )
    kalshi_private_call_concurrency: int = _get_int(
        "KALSHI_PRIVATE_CALL_CONCURRENCY", 3, min_value=1, max_value=16
    )
    kalshi_public_max_attempts: int = _get_int(
        "KALSHI_PUBLIC_MAX_ATTEMPTS", 8, min_value=1, max_value=20
    )
    kalshi_private_max_attempts: int = _get_int(
        "KALSHI_PRIVATE_MAX_ATTEMPTS", 8, min_value=1, max_value=20
    )
    kalshi_429_backoff_base_s: float = _get_float(
        "KALSHI_429_BACKOFF_BASE_S", 1.8, min_value=0.1
    )
    kalshi_429_backoff_cap_s: float = _get_float(
        "KALSHI_429_BACKOFF_CAP_S", 55.0, min_value=1.0
    )
    kalshi_429_jitter_s: float = _get_float("KALSHI_429_JITTER_S", 0.6, min_value=0.0)
    kalshi_5xx_backoff_base_s: float = _get_float(
        "KALSHI_5XX_BACKOFF_BASE_S", 0.4, min_value=0.05
    )
    kalshi_5xx_backoff_cap_s: float = _get_float(
        "KALSHI_5XX_BACKOFF_CAP_S", 8.0, min_value=0.5
    )
    kalshi_ws_ping_interval_s: float = _get_float(
        "KALSHI_WS_PING_INTERVAL_S", 20.0, min_value=5.0
    )
    kalshi_ws_ping_timeout_s: float = _get_float(
        "KALSHI_WS_PING_TIMEOUT_S", 45.0, min_value=5.0
    )
    kalshi_ws_close_timeout_s: float = _get_float(
        "KALSHI_WS_CLOSE_TIMEOUT_S", 5.0, min_value=1.0
    )
    kalshi_ws_recv_timeout_s: float = _get_float(
        "KALSHI_WS_RECV_TIMEOUT_S", 1.0, min_value=0.1
    )
    kalshi_ws_no_ticker_sleep_s: float = _get_float(
        "KALSHI_WS_NO_TICKER_SLEEP_S", 15.0, min_value=1.0
    )
    kalshi_ws_reconnect_base_s: float = _get_float(
        "KALSHI_WS_RECONNECT_BASE_S", 1.0, min_value=0.1
    )
    kalshi_ws_reconnect_multiplier: float = _get_float(
        "KALSHI_WS_RECONNECT_MULTIPLIER", 1.8, min_value=1.1
    )
    kalshi_ws_reconnect_jitter_s: float = _get_float(
        "KALSHI_WS_RECONNECT_JITTER_S", 0.35, min_value=0.0
    )
    startup_restore_ttls_delay_s: float = _get_float(
        "KALSHI_STARTUP_RESTORE_TTLS_DELAY_S", 8.0, min_value=0.0
    )
    prewarm_max_concurrent: int = _get_int(
        "KALSHI_PREWARM_MAX_CONCURRENT", 4, min_value=1, max_value=16
    )
    dual_engine_lab_tick_stagger_s: float = _get_float(
        "KALSHI_LAB_TICK_STAGGER_S", 0.45, min_value=0.0
    )
    dashboard_orderbook_cache_ttl_s: float = _get_float(
        "DASHBOARD_ORDERBOOK_CACHE_TTL_S", 5.0, min_value=0.5
    )
    ...
    engine_orderbook_enrich_first_tick_cap: int = _get_int(
        "ENGINE_ORDERBOOK_ENRICH_FIRST_TICK_CAP", 10, min_value=1
    )
    engine_orderbook_enrich_steady_cap: int = _get_int(
        "ENGINE_ORDERBOOK_ENRICH_STEADY_CAP", 20, min_value=1
    )
    ...
    default_paper_balance_cents: int = _get_int(
        "DEFAULT_PAPER_BALANCE_CENTS", 500_000, min_value=0
    )
    default_auto_close_open_sim_minutes: float = _get_float(
        "DEFAULT_AUTO_CLOSE_OPEN_SIM_MINUTES", 75.0, min_value=0.0
    )
