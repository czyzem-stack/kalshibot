...
class RuleMatchesTest(unittest.TestCase):
    def test_yes_band_center(self) -> None:
        r = {"name": "t", "min_prob": 0.5, "max_prob": 0.6, "min_minutes_left": 5.0, "max_minutes_left": 20.0}
    ...
    def test_rule_axis_probability(self) -> None:
        r_no = {"side": "no", "min_prob": 0.2, "max_prob": 0.8, "min_minutes_left": 0.0, "max_minutes_left": 99.0}
        self.assertAlmostEqual(rule_axis_probability(0.75, r_no), 0.25)
        r_yes = {"min_prob": 0.0, "max_prob": 1.0, "min_minutes_left": 0.0, "max_minutes_left": 99.0}
...
class PickTradeRuleTest(unittest.TestCase):
    def test_low_yes_cutoff_prefers_no_rules(self) -> None:
        ...
        yes_r = {"name": "y", "min_prob": 0.5, "max_prob": 0.9, "min_minutes_left": 0.0, "max_minutes_left": 30.0}
        # When implied YES is 0.30, axis for NO rules is ~0.70 — band must cover that.
        no_r = {"name": "n", "side": "no", "min_prob": 0.65, "max_prob": 0.85, "min_minutes_left": 0.0, "max_minutes_left": 30.0}
        rules = [yes_r, no_r]
    ...
    def test_no_book_blocks_no_rule(self) -> None:
        cfg = {}
        no_r = {"name": "n", "side": "no", "min_prob": 0.2, "max_prob": 0.9, "min_minutes_left": 0.0, "max_minutes_left": 30.0}
        yes_r = {"name": "y", "min_prob": 0.4, "max_prob": 0.6, "min_minutes_left": 0.0, "max_minutes_left": 30.0}
        rules = [yes_r, no_r]
        self.assertIsNone(
            pick_trade_rule(0.55, 10.0, rules, has_yes_rules=False, has_no_book=False, cfg=cfg),
        )
